...
$templatePath = Join-Path $scriptDir 'template_fidelidade.html'
$outputPath = Join-Path $scriptDir 'carrossel_fidelidade.html'

$images = @{
    'COVER_B64_HERE' = 'cover_jantar_casal_plano_sem_logo_1783691431293.png'
}
...
foreach ($placeholder in $images.Keys) {
    ...
    if (-not (Test-Path $imgFile)) {
        Write-Host "AVISO: Imagem pendente de geracao (pulando): $($images[$placeholder])" -ForegroundColor Yellow
        continue
    }
